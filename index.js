...
function openTab(evt, tabName) {
  ...
  if (isActive) { // Tab already active, so close it & hide everything
  ...
  } else { // No tabs active, so hide everything except the active tab
    for (i = 0; i < tab_content.length; i++) {
      tab_content[i].style.display = "none";
    }
    for (i = 0; i < tab_links.length; i++) {
    ...
    if (tabName === "Instructions") {
      document.getElementById(tabName).style.backgroundImage = image_encode;
